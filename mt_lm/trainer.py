...
from utils import collate_batch, mask_grad
...
def main(args):
    ...
    mt_corpus = MutationalCorpus(dataset_path, debug, 5)
    background = mt_corpus.background
    background = torch.from_numpy(background)
    masked_corpus = MaskedMutationalCorpus(mt_corpus, args.p, background, noise)
    ...
    masked_corpus_iter = DataLoader(masked_corpus, batch_size=batch_size, collate_fn=collate_batch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--num-steps', type=int, default=500, help='number of training steps')
